...
\begin{table}[t]
\centering
\caption{Continuous Non-Linear Manifold Benchmark Results}
\label{tab:continuous_benchmarks}
\resizebox{\columnwidth}{!}{
\begin{tabular}{lccccc}
\toprule
\textbf{Benchmark} & \textbf{Samples ($N$)} & \textbf{Seed Footprint} & \textbf{Accuracy} & \textbf{Precision / Recall} & \textbf{F1-Score} \\
\midrule
\textbf{Two-Moons} & $1000$ ($\sigma=0.10$) & 24 Bytes (1 Patch) & \textbf{99.30\%} & 99.01\% / 99.60\% & \textbf{99.30\%} \\
\textbf{Two-Spirals} & $200$ ($\sigma=0.04$) & 48 Bytes (2 Patches) & \textbf{98.50\%} & 100.0\% / 97.00\% & \textbf{98.48\%} \\
\bottomrule
\end{tabular}
}
\end{table}
